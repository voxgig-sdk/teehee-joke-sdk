...
[MCP](https://modelcontextprotocol.io) server exposing the TeeheeJoke SDK as
two agent tools — `teehee-joke_list` and `teehee-joke_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/teehee-joke-mcp)
make build

# 2. Provide credentials via the environment
export TEEHEE_JOKE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user teehee-joke \
  -- /absolute/path/to/teehee-joke-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./teehee-joke-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// teehee-joke_list: first page of records
{ "entity": "joke" }
{ "entity": "joke", "query": { } }

// teehee-joke_load: one record by id
{ "entity": "joke", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/teehee-joke-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user teehee-joke \
     -- "$PWD"/dist/*/teehee-joke-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `teehee-joke_list` and `teehee-joke_load` tools now appear
   in new sessions. Ask the agent to *"list joke using teehee-joke"*
   and it calls `teehee-joke_list` with `{"entity":"joke"}`.
...
```sh
export TEEHEE_JOKE_APIKEY=sk_live_xxx            # API key
export TEEHEE_JOKE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `teehee-joke_list` tool
...
### Call the `teehee-joke_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `teehee-joke_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `teehee-joke_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEEHEE_JOKE_APIKEY` | API key sent with every request. |
| `TEEHEE_JOKE_BASE` | Optional override of the API base URL. |
...
```sh
./teehee-joke-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"teehee-joke_load","arguments":{"entity":"joke","query":{"id":1}}}}'
```
