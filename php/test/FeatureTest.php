...
class FtCtrl extends TeeheeJokeControl
{
...
class FtClient
{
    ...
    private TeeheeJokeUtility $utility;

    public function __construct(TeeheeJokeUtility $utility, array $options)
    {
    ...
    public function get_utility(): TeeheeJokeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TeeheeJokeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TeeheeJokeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TeeheeJokeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TeeheeJokeConfig::make_feature($name);
    ...
    private static function buildUrl(TeeheeJokeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TeeheeJokeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TeeheeJokeResponse($fetched);
        }
    ...
    private function fail(TeeheeJokeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TeeheeJokeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
