...
class TeeheeJokeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TeeheeJokeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TeeheeJokeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TeeheeJokeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TeeheeJokeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TeeheeJokeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TeeheeJokeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TeeheeJokeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TeeheeJokeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TeeheeJokeError($op . "_allow",
                "TeeheeJokeSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TeeheeJokeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TeeheeJokeError("graphql_error",
                "TeeheeJokeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Joke($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_joke === null) {
                $this->_joke = new JokeEntity($this, null);
            }
            return $this->_joke;
        }
        return new JokeEntity($this, $data);
    }
