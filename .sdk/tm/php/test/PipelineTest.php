...
{
    private static function utility(): TeeheeJokeUtility
    {
        return new TeeheeJokeUtility();
    }
    ...
    private static function ctx(array $over = []): TeeheeJokeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TeeheeJokeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TeeheeJokeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TeeheeJokeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TeeheeJokeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TeeheeJokeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TeeheeJokeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TeeheeJokeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TeeheeJokeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TeeheeJokeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TeeheeJokeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TeeheeJokeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TeeheeJokeResponse([]),
            'result' => new TeeheeJokeResult([]),
        ]);
        [, $err] = TeeheeJokeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TeeheeJokeSpec([]),
            'response' => null,
            'result' => new TeeheeJokeResult([]),
        ]);
        [, $err] = TeeheeJokeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TeeheeJokeSpec(['step' => 's']),
            'response' => new TeeheeJokeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TeeheeJokeResult([]),
        ]);
        [, $err] = TeeheeJokeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TeeheeJokeSpec(['step' => 's']),
            'response' => new TeeheeJokeResponse(self::resp(200, ['v' => 1])),
            'result' => new TeeheeJokeResult([]),
        ]);
        [, $err] = TeeheeJokeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TeeheeJokeSpec(['step' => 's']),
            'response' => new TeeheeJokeResponse(self::resp(200, ['v' => 2])),
            'result' => new TeeheeJokeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TeeheeJokeMakeResponse::call($ctx);
    ...
    {
        $preset = new TeeheeJokeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TeeheeJokeSpec([]),
            'response' => new TeeheeJokeResponse([]),
            'result' => new TeeheeJokeResult([]),
        ...
        [$got, $err] = TeeheeJokeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TeeheeJokeResult([])]);
        [, $err] = TeeheeJokeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TeeheeJokeSpec([]), 'result' => null]);
        [, $err] = TeeheeJokeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TeeheeJokeSpec(['step' => 's']),
            'result' => new TeeheeJokeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TeeheeJokeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TeeheeJokeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TeeheeJokeSpec(['step' => 's']),
            'result' => new TeeheeJokeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TeeheeJokeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TeeheeJokeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TeeheeJokeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TeeheeJokeSpec([]),
            'result' => new TeeheeJokeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TeeheeJokeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TeeheeJokeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TeeheeJokeError('boom', 'boom');
        $utility->fetcher = function (TeeheeJokeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TeeheeJokeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TeeheeJokeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TeeheeJokeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TeeheeJokeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TeeheeJokeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        TeeheeJokeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TeeheeJokeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TeeheeJokeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TeeheeJokeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TeeheeJokeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TeeheeJokeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TeeheeJokeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TeeheeJokeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TeeheeJokeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TeeheeJokeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TeeheeJokeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TeeheeJokeResult(['ok' => false])]);
        $this->expectException(TeeheeJokeError::class);
        TeeheeJokeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TeeheeJokeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TeeheeJokeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TeeheeJokeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TeeheeJokeBaseFeature();
        $b = new TeeheeJokeBaseFeature();
        TeeheeJokeFeatureAdd::call($ctx, $a);
        TeeheeJokeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TeeheeJokeBaseFeature
    {
        $f = new TeeheeJokeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TeeheeJokeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TeeheeJokeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TeeheeJokeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TeeheeJokePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        TeeheeJokePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TeeheeJokeResponse(['headers' => null]),
            'result' => new TeeheeJokeResult([]),
        ]);
        TeeheeJokeResultHeaders::call($ctx);
