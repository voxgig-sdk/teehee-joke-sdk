...
from teeheejoke_sdk.teeheejoke_types import (
    Joke,
    JokeLoadMatch,
)
...
class JokeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return JokeEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Joke:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Joke().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
