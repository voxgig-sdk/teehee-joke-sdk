...
class TeeheeJokeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TeeheeJokeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TeeheeJokeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TeeheeJokeTestRunner.getenv("TEEHEE_JOKE_TEST_LIVE")
        override = TeeheeJokeTestRunner.getenv("TEEHEE_JOKE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TeeheeJokeTestRunner.getenv(key)
        ...
        explain = TeeheeJokeTestRunner.getenv("TEEHEE_JOKE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TEEHEE_JOKE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TeeheeJokeTestRunner._test_control is not None:
            return TeeheeJokeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TeeheeJokeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TeeheeJokeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TeeheeJokeTestRunner.load_test_control()
...
def load_env_local():
    TeeheeJokeTestRunner.load_env_local()


def env_override(m):
    return TeeheeJokeTestRunner.env_override(m)


def entity_data(v):
    return TeeheeJokeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TeeheeJokeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TeeheeJokeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TeeheeJokeTestRunner.load_test_control()


def live_client_options():
    return TeeheeJokeTestRunner.live_client_options()


def live_delay_ms():
    return TeeheeJokeTestRunner.live_delay_ms()
