...
from teeheejoke_sdk.utility.voxgig_struct import voxgig_struct as vs
from teeheejoke_sdk import TeeheeJokeSDK
from teeheejoke_sdk.core.spec import TeeheeJokeSpec
from teeheejoke_sdk.core.result import TeeheeJokeResult
from teeheejoke_sdk.core.response import TeeheeJokeResponse
from teeheejoke_sdk.core.operation import TeeheeJokeOperation
from teeheejoke_sdk.core.error import TeeheeJokeError
from teeheejoke_sdk.core import helpers
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return TeeheeJokeError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = TeeheeJokeSDK.test(None, None)
        ...
        class TestInitFeature(TeeheeJokeBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(TeeheeJokeBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = TeeheeJokeSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = TeeheeJokeSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = TeeheeJokeSDK.test(None, setup_opts)
