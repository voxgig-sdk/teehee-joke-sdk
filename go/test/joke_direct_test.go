...
func TestJokeDirect(t *testing.T) {
	t.Run("direct-load-joke", func(t *testing.T) {
		setup := jokeDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-joke", _mode); _shouldSkip {
			if _reason == "" {
...
func jokeDirectSetup(mockres any) *jokeDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"TEEHEE_JOKE_TEST_JOKE_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewTeeheeJokeSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["TEEHEE_JOKE_TEST_JOKE_ENTID"]; ok {
