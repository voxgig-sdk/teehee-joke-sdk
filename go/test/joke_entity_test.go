...
func TestJokeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		jokeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.joke")))
...
func jokeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TEEHEE_JOKE_TEST_JOKE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TEEHEE_JOKE_TEST_JOKE_ENTID"])
