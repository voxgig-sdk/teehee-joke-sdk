...
import (
	...
	omni "github.com/voxgig-sdk/teehee-joke-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TeeheeJokeError); is {
				out := map[string]any{
					"name":    "TeeheeJokeError",
...
func omniCtx(arg any, client *sdk.TeeheeJokeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TeeheeJokeSDK); is {
			client = live
