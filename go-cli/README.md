...
boru-driven command-line client **and** interactive REPL for the TeeheeJoke
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/teehee-joke-cli)
make build

# 2. See usage (words, entities, env vars)
./teehee-joke-cli --help

# 3. Provide credentials once, via the environment
export TEEHEE_JOKE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./teehee-joke-cli load 1 joke            # {id:1} shorthand
./teehee-joke-cli load '{id:1}' joke       # explicit match map

# 5. Override the API base URL for a single call
TEEHEE_JOKE_BASE=https://api.example.com ./teehee-joke-cli load 1 joke

# 6. No arguments -> interactive REPL
./teehee-joke-cli
teehee-joke> load 1 joke
teehee-joke> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/teehee-joke-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/teehee-joke-cli load 1 joke
   ```
...
```sh
./teehee-joke-cli load 1 joke          # scalar shorthand for {id:1}
./teehee-joke-cli load '{id:1}' joke     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `teehee-joke>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `joke`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/teehee-joke-cli`. |
...
(`./teehee-joke-cli <expr>`) and interactively (the REPL), and expressions compose the
