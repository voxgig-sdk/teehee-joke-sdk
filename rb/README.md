...
The Ruby SDK for the TeeheeJoke API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Joke` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = TeeheeJokeSDK.test(testopts, sdkopts)
```
...
raise a `TeeheeJokeError` (a `StandardError` subclass), so wrap
...
```
rb/
├── TeeheeJoke_sdk.rb       -- Main SDK module
...
The main module (`TeeheeJoke_sdk`) exports the SDK class
