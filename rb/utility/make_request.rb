...
module TeeheeJokeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TeeheeJokeResponse.new({})
    ...
    if fetch_err
      ...
      response = TeeheeJokeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TeeheeJokeResponse.new(fetched)
    else
