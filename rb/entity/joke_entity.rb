...
class JokeEntity
  ...
  def make
    opts = @_entopts.dup
    JokeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TeeheeJokeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TeeheeJokeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Joke.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Joke, Hash] the loaded Joke; raises TeeheeJokeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TeeheeJokeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
