...
class JokeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = joke_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "joke." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TEEHEE_JOKE_TEST_JOKE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    joke_ref01_data = nil
    if joke_ref01_data_raw.length > 0
...
def joke_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "joke", "JokeTestData.json")
  ...
  entid_env_raw = ENV["TEEHEE_JOKE_TEST_JOKE_ENTID"]
  ...
  if env["TEEHEE_JOKE_TEST_LIVE"] == "TRUE"
    merged_opts = Vs.merge([
      {
      },
      extra || {},
    ])
    client = TeeheeJokeSDK.new(Helpers.to_map(merged_opts))
  end

  live = env["TEEHEE_JOKE_TEST_LIVE"] == "TRUE"
  {
    client: client,
    data: entity_data,
