...
//     (\`new TeeheeJokeSDK(...)\` / \`TeeheeJokeSDK.test(...)\` become
//     \`TeeheeJokeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"joke":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TeeheeJokeSDK(...)\` and any
// \`TeeheeJokeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TeeheeJokeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TeeheeJokeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
