...
local JokeEntity = {}
JokeEntity.__index = JokeEntity


function JokeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JokeEntity)
  self._name = "joke"
  self._client = client
...
function JokeEntity:get_name()
  return self._name
end


function JokeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JokeEntity.new(self._client, opts)
end
...
function JokeEntity:mark_deleted()
  self._deleted = true
end


function JokeEntity:deleted()
  return true == self._deleted
end


function JokeEntity:data_set(args)
  if args ~= nil then
...
function JokeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JokeEntity:match_set(args)
  if args ~= nil then
...
function JokeEntity:stream(action, args, callopts)
...
---@param reqmatch JokeLoadMatch
---@param ctrl? table
---@return Joke
---@return string? err
function JokeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function JokeEntity:_run_op(ctx, post_done)
  local utility = self._utility
